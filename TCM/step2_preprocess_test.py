...
def load_lidc_xml(xml_path, agreement_threshold=0, only_patient=None, save_nodules=False):
    ...
    df_annos.to_csv(settings.TRAIN_EXTRACTED_IMAGE_DIR + "_labels/" + patient_id + "_annos_pos_lidc.csv", index=False)
    df_neg_annos = pandas.DataFrame(neg_lines, columns=["anno_index", "coord_x", "coord_y", "coord_z", "diameter", "malscore"])
    df_neg_annos.to_csv(settings.TRAIN_EXTRACTED_IMAGE_DIR + "_labels/" + patient_id + "_annos_neg_lidc.csv", index=False)
...
def process_excluded_annotations_patient(src_path, patient_id):
    ...
    pos_annos_df = pandas.read_csv(settings.TRAIN_EXTRACTED_IMAGE_DIR + "_labels/" + patient_id + "_annos_pos.csv")
    ...
    df_annos.to_csv(settings.TRAIN_EXTRACTED_IMAGE_DIR + "_labels/" + patient_id + "_annos_excluded.csv", index=False)
    return [patient_id, spacing[0], spacing[1], spacing[2]]
...
def process_auto_candidates_patient(src_path, patient_id, sample_count=1000, candidate_type="white"):
    dst_dir = settings.TRAIN_EXTRACTED_IMAGE_DIR + "/_labels/"
    img_dir = settings.TRAIN_EXTRACTED_IMAGE_DIR + patient_id + "/"
    df_pos_annos = pandas.read_csv(dst_dir + patient_id + "_annos_pos.csv")
...
def process_images(delete_existing=False, only_process_patient=None):
    if delete_existing and os.path.exists(settings.TEST_EXTRACTED_IMAGE_DIR):
        print("Removing old stuff..")
        if os.path.exists(settings.TEST_EXTRACTED_IMAGE_DIR):
            shutil.rmtree(settings.TEST_EXTRACTED_IMAGE_DIR)

    if not os.path.exists(settings.TEST_EXTRACTED_IMAGE_DIR):
        os.mkdir(settings.TEST_EXTRACTED_IMAGE_DIR)
        os.mkdir(settings.TEST_EXTRACTED_IMAGE_DIR + "_labels/")
...
def process_pos_annotations_patient2():
    ...
    for subject_no in range(settings.TRAIN_SUBSET_START_INDEX, settings.TRAIN_SUBSET_TRAIN_NUM):
        ...
        for src_path in glob.glob(src_dir + "*.mhd"):
            ...
            if patient_id=='LKDS-00395' or patient_id == 'LKDS-00434':
                continue
...
def process_auto_candidates_patients():
    for subject_no in range(settings.TRAIN_SUBSET_START_INDEX,settings.TRAIN_SUBSET_TRAIN_NUM):
        ...
        for patient_index, src_path in enumerate(glob.glob(src_dir + "*.mhd")):
            ...
            process_auto_candidates_patient(src_path, patient_id, sample_count=100, candidate_type="edge")
