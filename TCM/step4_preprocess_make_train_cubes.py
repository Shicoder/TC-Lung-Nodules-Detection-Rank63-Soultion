...
def make_annotation_images_lidc():
    ...
    for patient_index, csv_file in enumerate(glob.glob(src_dir + "*_annos_pos.csv")):
        ...
        if len(df_annos) == 0:
            continue
        images = helpers.load_patient_images(patient_id, settings.TRAIN_EXTRACTED_IMAGE_DIR, "*" + CUBE_IMGTYPE_SRC + ".png")
...
def make_candidate_auto_images(candidate_types=[]):
    ...
    for candidate_type in candidate_types:
        ...
        for index, csv_file in enumerate(glob.glob(src_dir + "*_candidates_" + candidate_type + ".csv")):
            ...
            if len(df_annos) == 0:
                continue
            images = helpers.load_patient_images(patient_id, settings.TRAIN_EXTRACTED_IMAGE_DIR, "*" + CUBE_IMGTYPE_SRC + ".png", exclude_wildcards=[])
...
def make_pos_annotation_images_manual_ndsb3():
    ...
    for patient_index, csv_file in enumerate(glob.glob(src_dir + "*.csv")):
        ...
        if len(df_annos) == 0:
            continue
        images = helpers.load_patient_images(patient_id, settings.TEST_EXTRACTED_IMAGE_DIR, "*" + CUBE_IMGTYPE_SRC + ".png")
