...
def rescale_patient_images(images_zyx, org_spacing_xyz, target_voxel_mm, is_mask_image=False, verbose=True):
    if verbose:
...
def percentage_to_orig(x_perc,x_space, y_perc,y_space, z_perc,z_space, cube_image):
    res_x = int(round(x_perc * cube_image.shape[2]/x_space))
    res_y = int(round(y_perc * cube_image.shape[1]/y_space))
    res_z = int(round(z_perc * cube_image.shape[0]/z_space))
    return res_x, res_y, res_z
PATIENT_LIST = None
