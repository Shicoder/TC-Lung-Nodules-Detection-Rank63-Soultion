...
def filter_patient_nodules_predictions(df_nodule_predictions: pandas.DataFrame, patient_id, view_size, luna16=False):
    src_dir = settings.TRAIN_EXTRACTED_IMAGE_DIR if luna16 else settings.TEST_EXTRACTED_IMAGE_DIR
...
def make_negative_train_data_based_on_predicted_luna_nodules():
    src_dir = settings.TRAIN_NODULE_DETECTION_DIR+'predictions10_luna16_fs/'
    pos_labels_dir = settings.TRAIN_EXTRACTED_IMAGE_DIR+'_labels/'
    ...
    for csv_index, csv_path in enumerate(glob.glob(src_dir + "*.csv")):
        ...
        manual_path = settings.TRAIN_EXTRACTED_IMAGE_DIR+'_labels/' + patient_id + "_candidates_edge.csv"
        ...
        pos_labels = pandas.read_csv(pos_labels_dir + patient_id + "_annos_pos.csv")
        print(csv_index, ": ", patient_id, ", pos", len(pos_labels))
        patient_imgs = helpers.load_patient_images(patient_id, settings.TRAIN_EXTRACTED_IMAGE_DIR, "*_m.png")
        for nod_pred_index, nod_pred_row in df_nodule_predictions.iterrows():
            ...
            if pos_annos_manual is not None:
                for index, label_row in pos_annos_manual.iterrows():
                    px, py, pz = helpers.percentage_to_pixels(label_row["coord_x"], label_row["coord_y"], label_row["coord_z"], patient_imgs)
                    diameter = label_row["diameter"] * patient_imgs[0].shape[1]
...
def make_predicted_luna_nodules():
    ...
    for csv_index, csv_path in enumerate(glob.glob(src_dir + "*.csv")):
        ...
        patient_imgs = helpers.load_patient_images(patient_id, settings.TEST_EXTRACTED_IMAGE_DIR, "*_m.png")
        ...
        df_nodule_predictions.sort_values(by='nodule_chance',ascending = False,inplace=True)
        ...
        for nod_pred_index, nod_pred_row in df_nodule_predictions_copy.iterrows():
            ...
            for index, row in df_nodule_predictions.iterrows():
                ...
                if dist < (candidate_diameter + 48) and dist> 1: #  make sure we have a big margin
                    ok = False
                    print("# Too close")
                    mal_score = row["diameter_mm"]
...
def merge_csv():
    df_empty = pandas.DataFrame(columns=['seriesuid','coord_x','coord_y','coord_z','nodule_chance',])
    src_dir =settings.TEST_NODULE_DETECTION_DIR
    ...
    df_empty.rename(columns={'coord_x':'coordX','coord_y':'coordY','coord_z':'coordZ','nodule_chance':'probability'}, inplace = True)
    df_empty.to_csv("prediction_can.csv",index=False)


#
# def get_patient_xyz_do(src_path, patient_id):
#     df_node = pandas.read_csv("prediction_can.csv")
#
...
#
#     patient_imgs = helpers.load_patient_images(patient_id, settings.NDSB3_EXTRACTED_IMAGE_DIR, "*_i.png")
#
#     pos_annos = []
#     df_patient = df_node[df_node["seriesuid"] == patient_id]
#     anno_index = 0
...
#     only_patient = None
#     patient_list=[]
#     for subject_no in range(settings.LUNATEST_SUBSET_START_INDEX, settings.LUNATEST_SUBSET_TRAIN_NUM):
#         src_dir = settings.LUNA16_RAW_SRC_DIR + "test_subset0" + str(subject_no) + "/"
...
#             if  patient_id == 'LKDS-00384' or patient_id == 'LKDS-00186':
#                 continue
#             pos_annos = get_patient_xyz_do(src_path, patient_id)
...
#     df_patient_list.to_csv("prediction_submission.csv", index=False)
...
def get_patient_xyz_do(src_path, patient_id,f_path):
    ...
    patient_imgs = helpers.load_patient_images(patient_id, settings.TEST_EXTRACTED_IMAGE_DIR, "*_i.png")
...
def merge_csv_orign():
    ...
    print('Len of TOTAL:',len(df_empty))
    df_empty.to_csv("./output_val/prediction_can.csv",index=False)
...
def predict_cubes(model_path, continue_job, only_patient_id=None, luna16=False, magnification=1, flip=False, train_data=True, holdout_no=-1, ext_name="", fold_count=2):
    if luna16:
        ...
        extract_path = settings.TRAIN_EXTRACTED_IMAGE_DIR
    else:
        dst_dir = settings.TEST_NODULE_DETECTION_DIR
        extract_path = settings.TEST_EXTRACTED_IMAGE_DIR
    ...
    for file_name in os.listdir(extract_path):
        if not os.path.isdir(extract_path + file_name):
            continue
    ...
    for patient_index, patient_id in enumerate(reversed(patient_ids)):
        ...
        patient_img = helpers.load_patient_images(patient_id, extract_path, "*_i.png", [])
        if magnification != 1:
            patient_img = helpers.rescale_patient_images(patient_img, (1, 1, 1), magnification)

        patient_mask = helpers.load_patient_images(patient_id, extract_path, "*_m.png", [])
        if magnification != 1:
        ...
        filter_patient_nodules_predictions(df, patient_id, CROP_SIZE * magnification,luna16)
...
if __name__ == "__main__":
    ...
    if False:
        for magnification in [1]:  #
            ...
            # predict_cubes("workdir/model_luna16_full__fs_best.hd5", CONTINUE_JOB, only_patient_id=only_patient_id,luna16 =True, magnification=magnification, flip=False, train_data=False, holdout_no=None, ext_name="luna16_fs")
    ...
    merge_csv_orign()
    ###################
    path_f = './output_val/prediction_can.csv'
